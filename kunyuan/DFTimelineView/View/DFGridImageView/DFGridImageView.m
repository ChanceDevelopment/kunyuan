...
-(void)updateWithImages:(NSMutableArray *)images srcImages:(NSMutableArray *)srcImages oneImageWidth:(CGFloat)oneImageWidth oneImageHeight:(CGFloat)oneImageHeight
{
    ...
    if (images.count == 1) {
        ...
        id imageData = [images objectAtIndex:0];
        if ([imageData isKindOfClass:[UIImage class]]) {
            [_oneImageView setImage:imageData];
        }
        else{
    ...
    for (int i=0; i< _imageViews.count; i++) {
        ...
        if (images.count == 1) {
            ...
            if (images.count == 4) {
                if (i == 0 || i == 1 ) {
                    id imageData = [images objectAtIndex:i];
                    if ([imageData isKindOfClass:[UIImage class]]) {
                        [imageUnitView.imageView setImage:imageData];
                    }
                    else{
                    ...
                    id imageData = [images objectAtIndex:i - 1];
                    if ([imageData isKindOfClass:[UIImage class]]) {
                        [imageUnitView.imageView setImage:imageData];
                    }
                    else{
...
-(void) onClickImage:(UIView *) sender
{
    ...
    if (self.srcImages.count > 1) {
        ...
        for (int i=0; i<self.images.count; i++) {
            ...
            id imageData = [self.srcImages objectAtIndex:i];
            if ([imageData isKindOfClass:[UIImage class]]) {
                photo.image = imageData;
            }
            else{
                photo.url = [NSURL URLWithString:imageData];
            }
...
//            photo.url = [NSURL URLWithString:[self.srcImages objectAtIndex:i]];
        ...
        MJPhoto *photo = [[MJPhoto alloc] init];
        id imageData = [self.srcImages objectAtIndex:0];
        if ([imageData isKindOfClass:[UIImage class]]) {
            photo.image = imageData;
        }
        else{
